...
// Fallback seed data in case the database table doesn't exist yet
const SEED_PLANS = [
  { 
    id: '1', 
...
export async function GET(req: Request) {
  try {
    ...
    if (error) {
      console.warn("Supabase query for membership_plans failed, using local seed data:", error);
      return NextResponse.json(SEED_PLANS);
    }

    // If query succeeds but table is empty, return seed data
