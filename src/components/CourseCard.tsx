    ...
    <Link href={`/courses/${id}`} className="group bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition duration-300">
      <div className="relative aspect-video">
        ...
        <div className="absolute top-2 left-2">
          <span className="bg-[var(--brand)] text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider">
            {category}
          </span>
        </div>
      </div>
      <div className="p-4">
        <h3 className="text-gray-900 font-bold text-lg mb-1 line-clamp-2 leading-snug group-hover:text-[var(--brand)] transition">
          {title}
        </h3>
        {instructor && <p className="text-gray-500 text-sm mb-3">講師：{instructor}</p>}
        <div className="flex items-center justify-between mt-auto">
          <span className="text-[var(--brand)] font-bold text-xl">
