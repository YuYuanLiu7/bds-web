...
本文件提供 BDS 線上學習平台（基於 Next.js 構建）極為詳細的技術規格與系統架構說明。本說明書以**功能為導向**，詳細載明 BDS 平台目前已實作與預期具備的前後端功能，適合直接提供給 `claude-cli` 或其他 AI 助理進行 **Code Review (代碼審查)** 與 **功能驗證 (Validation)**。
...
│   │   ├── admin/                     # 管理員後台頁面
│   │   │   ├── articles/              # 專欄文章上架與付費牆設定
│   │   │   ├── assets/                # 素材庫與本地上傳檔案管理 (整合上傳端點)
│   │   │   ├── comments/              # 課堂留言審核管理 (使用 Local Storage 同步)
│   │   │   ├── courses/               # 課程管理與章節編輯 (包含學員名單檢視)
│   │   │   │   └── [id]/students/     # 課程專屬學員名單
│   │   │   ├── developer/             # 開發者工具 (除錯日誌、系統重設與資料種子)
│   │   │   ├── downloads/             # 數位資源下載項目 CRUD 編輯
│   │   │   ├── events/                # 線上實戰營與線下講座活動管理
│   │   │   ├── finance/               # 財務報表與訂單歷史紀錄
│   │   │   ├── global-settings/       # 網站全域視覺設定
│   │   │   ├── help/                  # 後台系統幫助指南
│   │   │   ├── integrations/          # 第三方追蹤碼與發信 API 整合面板
│   │   │   ├── marketing/             # 行銷工具 (優惠券/折扣與活動推廣)
│   │   │   ├── media/                 # 媒體庫重新導向元件
│   │   │   ├── membership/            # 會員方案設定頁面
│   │   │   ├── news/                  # 學習頁面動態公告發布
│   │   │   ├── pages/                 # 自訂頁面內容編輯器 (關於/聯絡/條款)
│   │   │   ├── rewards/               # 推薦行銷與分潤設定頁面 (固定 5% 分潤)
│   │   │   ├── settings/              # 系統基本金流配置設定
│   │   │   ├── students/              # 全站學員名單與課程權限手動變更
│   │   │   └── page.tsx               # 後台儀表板主頁
│   │   ├── api/                       # 後端 REST API 路由
│   │   │   ├── auth/                  # 身分驗證相關 API
...
│   │   ├── about/                     # 關於我們頁面 (讀取動態設定)
│   │   ├── contact/                   # 聯絡我們頁面 (讀取動態設定與表單提交)
│   │   ├── help/                      # 常見問題 FAQ 幫助中心
│   │   ├── privacy/                   # 服務條款與隱私權政策條約頁面
│   │   ├── login/                     # 登入頁面
│   │   ├── signup/                    # 註冊頁面
...
* **驗證重點**：
  * 安全漏洞防範：文章被屏蔽時，其 `content` (文章詳細內文) 數據**絕對不能**以 any 形式渲染在 DOM 中（防止有心人使用 F12 審查元素直接繞過付費牆閱讀）。
...
* **核心功能**：
  ...
  * **選購與下載攔截彈窗 (DownloadsList)**：
    ...
    * **管理員特權**：若 session role 為 `'admin'`，彈窗上方會顯示紫色皇冠標章「👑 管理員最高特權已啟用」，點擊「管理員直接下載」可直接繞過 any 限制取得檔案。若後台尚未配置 `file_url`，也會自動為管理員下載樣品測試 PDF。
* **驗證重點**：
  * 驗證一般學員無法直接以直接調用 file_url 的方式繞過前端下載按鈕檢查（防盜鏈與付費控制機制）。

### 4.9 分類課程專區 (`/categories/[id]`)
* **頁面定位**：基於特定主題分類顯示篩選課程。
* **核心功能**：
  * 自動解析動態網址 `[id]`，並映射為人類易讀分類（如: `novice` -> `業務新手村`、`job` -> `職場升級`、`bookclub` -> `讀書會`、`fireside` -> `爐邊對談`）。
  * 僅拉取並顯示符合該分類之動態發布課程。
...
後台所有路由（位於 `/admin/**/*`）與對應的 API 端點，皆會強制進行身分檢驗，確保只有 `users.role === 'admin'` 的登入帳號方能操作，否則回傳 `401 Unauthorized` 或 `403 Forbidden`。

### 5.1 後台儀表板首頁 (`/admin`)
* **核心功能**：
  * **核心數據卡片**：統計累積訂單實收總金額（只加總 `orders.status = 'paid'` 的實收款項）、已註冊學生總人次。
  * **捷徑選單**：快速跳轉各管理模組。

### 5.2 課程與章節管理 (`/admin/courses` & `/admin/courses/[id]`)
* **核心功能**：
  * **課程列表統計**：展示全站課程。為提升效能，API (`/api/admin/courses_full`) 採用 `Promise.all` 平行抓取，為每堂課程附帶加總「直接購課學員數 + 當前 active 會員方案訂閱人數」的總學員數、以及該課程的累積淨銷售額 (Net Sales)。
  * **課程新增編輯 (CourseModal)**：可調整課程基本資料、定價、講師名稱、封面圖、Trailer 影片 URL、課程講義網址，並可開關是否允許學員評分、是否前台隱藏 (`is_hidden`)。
  * **單元編輯與排序**：在 `/admin/courses/[id]` 下可增減章節，填寫章節 title、影片網址與檔案講義連結，並可自由拖拽/輸入 order_index 調整順序。
  * **課程學員名單 (`/admin/courses/[id]/students`)**：檢視單門課程的購買學員名冊、註冊時間及信箱。

### 5.3 專欄文章管理 (`/admin/articles`)
* **核心功能**：
  * **文章 CRUD**：管理與新增專欄文章。可撰寫內文（支援 HTML 及 Markdown）、 Slug 網址代稱、上傳封面圖、設定發布狀態。
  * **付費牆設定**：下拉選單可控制 visibility 為 `public`、`members` 或 `course_purchasers`。若為課程學員專屬，可勾選指定的一門或多門課程，將其 ID 存入 `required_course_ids`。

### 5.4 活動講座管理 (`/admin/events`)
* **核心功能**：
  * **活動 CRUD**：管理實體與線上講座。設定開辦時間、報名票價與顯示價格、主視覺 banner。
  * **屬性設定**：設定活動為線上/線下型態，並提供 status 控制項（upcoming/completed）切換前台頁籤。可填入 `registration_url` 連接 Zoom 或外部金流。

### 5.5 數位下載管理 (`/admin/downloads`)
* **核心功能**：
  * **下載資源 CRUD**：管理下載專區的履歷、白皮書。設定其免費/付費定價、檔案分類標籤、下載次數手動調整，以及配置真實下載檔案的 file_url。

### 5.6 會員方案管理 (`/admin/membership`)
* **核心功能**：
  * **方案 CRUD**：建立訂閱方案（如月繳 NT$ 399、年繳 NT$ 3,600）。
  * 支援設定方案價格、付款週期（period）、特色打勾清單、是否設為 popular 推薦方案。
...
* **核心功能**：
  * **本地檔案上傳**：提供點選或拖拽檔案上傳（調用 `/api/admin/upload`）。
  * **文件瀏覽與刪除**：列出全站已上傳的圖片、PDF 講義等，可一鍵複製 CDN/存儲連結直接應用於課程講義或封面，並提供刪除檔案釋放空間功能。
