...
// 進度／對照表固定放專案根目錄(ROOT)，不用 process.cwd()：換資料夾執行才不會讀不到舊進度而重複上傳。
const PROGRESS = path.join(ROOT, 'materials-upload-progress.json');
const PLAN_CSV = path.join(ROOT, 'materials-upload-plan.csv');
const RESULT_CSV = path.join(ROOT, 'materials-upload-result.csv');
...
// 穩定檔名：以「相對路徑雜湊」命名，同一個檔重跑產生同一個物件名，
// 讓 upsert 真正冪等（避免像先前用 Date.now()+隨機碼每次都變新名、在 Storage 堆重複檔）。
function storedNameFor(f) {
  const h = crypto.createHash('sha1').update(f.rel).digest('hex').slice(0, 16);
  return `material-${h}.${f.ext}`;
}
...
(async () => {
  ...
  if (oversize.length) {
    warn(`其中 ${oversize.length} 個超過 50MB，將無法上傳（會標記在對照表中）：`);
    oversize.forEach((f) => console.log(`     · ${f.rel}（${mb(f.size)}）`));
    console.log('     → 太大的請先壓縮/拆檔，或放雲端後在後台貼連結。');
  }
  ...
  let done = 0, uploaded = 0, skipped = 0, failed = 0, tooLarge = 0;

  for (const f of files) {
    if (done >= LIMIT) break;
    done++;
    if (progress[f.full]?.url) { skipped++; continue; }
    if (f.size > MAX_BYTES) { warn(`略過（超過 50MB）：${f.rel}（${mb(f.size)}）`); tooLarge++; continue; }
    try {
      const buffer = fs.readFileSync(f.full);
      const storedName = storedNameFor(f);
      ...
      const publicUrl = supabase.storage.from('uploads').getPublicUrl(storedName).data.publicUrl;
  ...
  // 對照表：由「全部檔案 × 進度」重建（含超大而略過者），與本次是否 --limit 無關。
  const resultLines = ['course_guess,material_name,relative_path,status,public_url'];
  for (const f of files) {
    const doneUrl = progress[f.full]?.url;
    let status, pub;
    if (doneUrl) { status = 'uploaded'; pub = doneUrl; }
    else if (f.size > MAX_BYTES) { status = `skipped-too-large(${mb(f.size)})`; pub = ''; }
    else { status = 'not-uploaded'; pub = ''; }
    resultLines.push([f.guessCourse, f.name, f.rel, status, pub].map(csvCell).join(','));
  }
  ...
  if (tooLarge) warn(`因超過 50MB 而略過：${tooLarge} 個（已在對照表標記 skipped-too-large，需另行處理）`);
  if (failed) warn(`失敗：${failed} 個（可直接重跑，已成功的會自動略過）`);
  ok(`對照表已輸出：${RESULT_CSV}（status 欄可看出哪些已上傳、哪些超大略過、哪些尚未上傳）`);
